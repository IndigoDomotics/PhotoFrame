		...
		try:
			while True:
				if self.timeWarpOn:
					...
					keyValueList = []
					keyValueList.append({'key':'serverTimeSeconds', 'value':self.timeWarpCount})
					keyValueList.append({'key':'serverSecondsEven', 'value':not bool(self.timeWarpCount % 2)})
					self.serverTimeDev.updateStatesOnServer(keyValueList)
					self.sleep(0.15)
				else:
					...
					keyValueList = []
					keyValueList.append({'key':'serverTimeSeconds', 'value':serverTime.second})
					keyValueList.append({'key':'serverSecondsEven', 'value':not bool(serverTime.second % 2)})
